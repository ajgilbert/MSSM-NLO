...
parser.add_argument('--mssm-mode', action='store_true')
...
for p in ['s', 'A', 'H']:
    ...
    if args.mssm_mode:
        if P is not 'A':
            wsptools.SafeWrapHist(w, ['mA', 'tanb'], GetFromTFile('mhmodp_mu200_13TeV.root:m_%s' % P), name='m%s' % P)
    ...
    if not args.mssm_mode:
        w.factory('m%s[%g,%g]' % (P, min(masses_f), max(masses_f)))

    spline_xsec_t_2HDM_ref = ROOT.RooSpline1D('gg%s_t_2HDM_xsec' % (P), '', w.arg('m%s' % (P)), len(masses_f), array('d', masses_f), array('d', xsec_t_2HDM_ref), 'LINEAR')
    spline_xsec_b_2HDM_ref = ROOT.RooSpline1D('gg%s_b_2HDM_xsec' % (P), '', w.arg('m%s' % (P)), len(masses_f), array('d', masses_f), array('d', xsec_b_2HDM_ref), 'LINEAR')
    spline_xsec_i_2HDM_ref = ROOT.RooSpline1D('gg%s_i_2HDM_xsec' % (P), '', w.arg('m%s' % (P)), len(masses_f), array('d', masses_f), array('d', xsec_i_2HDM_ref), 'LINEAR')
    ...
    for m in masses:
        ...
        d = dist
        di = dist # special for the interference part which may be stats limited
        if m in use_dist[p]:
            di = use_dist[p][m]
        ...
        wsptools.SafeWrapHist(w, ['h_pt'],
            GetNormed('output_%s_%s_%s.root:analysis/%s/int' % (p, m, T, di)), name='%s_%s_i' % (P, m))
